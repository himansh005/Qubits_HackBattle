...
peers = set()
print("___________________________Started Server________________________")
...
def signup():
    ...
    #     if not len(tx_data.get(field))==0:
...
def new_transaction():
    tx_data = request.get_json()
    # print(tx_data)
    required_fields = ["username", "password","content","coins"]
    ...
    tx_data["timestamp"] = time.time()
    tx_data["coins"]=tx_data["coins"]+0.02
    # print(tx_data.get("coins"))
    blockchain.add_new_transaction(tx_data)
    mine_unconfirmed_transactions()
    return "Success", 201
